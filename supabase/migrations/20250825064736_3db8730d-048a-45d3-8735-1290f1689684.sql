...
CREATE TABLE public.profiles (
  ...
  email TEXT,
  phone TEXT,
  role TEXT DEFAULT 'guest' CHECK (role IN ('shg_admin', 'member', 'guest')),
  shg_id UUID,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Create shgs table for Self-Help Groups
...
CREATE TABLE public.shg_track_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  shg_id UUID REFERENCES public.shgs(id) ON DELETE CASCADE,
...
-- Add foreign key constraint for profiles.shg_id
ALTER TABLE public.profiles 
ADD CONSTRAINT profiles_shg_id_fkey 
FOREIGN KEY (shg_id) REFERENCES public.shgs(id);

-- Create geospatial index for location-based queries
CREATE INDEX idx_shgs_location ON public.shgs USING GIST (location);
...
ALTER PUBLICATION supabase_realtime ADD TABLE public.shgs;
ALTER PUBLICATION supabase_realtime ADD TABLE public.shg_members;
ALTER PUBLICATION supabase_realtime ADD TABLE public.shg_track_records;
